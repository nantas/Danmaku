...
#pragma strict
#pragma downcast
@CustomEditor(BulletPattern)


class BulletPatternEditor extends Editor 
{
	var bp : BulletPattern;
	var bm : BulletManager;
	...
	function FireTagsGUI () 
	{
		if (bp.fireTags == null)
			bp.fireTags = new FireTag[0];
			
		var fireTags = new List.<FireTag>(bp.fireTags);
		
		if (fireTags.Count != bp.ftFoldouts.Count)
			bp.ftFoldouts = new List.<boolean>(new boolean[fireTags.Count]);
		...
		if (GUILayout.Button("Collapse All", GUILayout.Width(150)))
		{
			...
			for(zz = 0; zz < bp.ftFoldouts.Count;zz++)
	...
	function FireTagActionsGUI(i)
	{
		...
		if (actions.Count != bp.ftaFoldouts[i].sub.Count)
			bp.ftaFoldouts[i].sub = new List.<boolean>(new boolean[actions.Count]);
		...
		if (bp.ftaFoldouts[i].main ) 
		{
			...
			for (var l=0; l<actions.Count; l++) 
			{
				...
				if (bp.ftaFoldouts[i].sub[l]) 
				{
					...
					ac.type = EditorGUILayout.EnumPopup("Action Type", ac.type);
					...
					switch(ac.type)
					{
						...
						case(FireActionType.Fire):
							ac.direction = EditorGUILayout.EnumPopup("DirectionType", ac.direction);
							if(!ac.useParam)
							{
								GUILayout.BeginHorizontal();
								if(!ac.randomAngle)
									ac.angle.x = EditorGUILayout.IntField("Angle", ac.angle.x);
								else
						...
						case(FireActionType.StartRepeat):
							ac.repeatCount.x = EditorGUILayout.IntField("RepeatCount", ac.repeatCount.x);
							GUILayout.BeginHorizontal();
	...
	function BulletTagsGUI () 
	{
		...
		if (bulletTags.Count != bp.btFoldouts.Count)
			bp.btFoldouts = new List.<boolean>(new boolean[bulletTags.Count]);
		...
		if (GUILayout.Button("Collapse All", GUILayout.Width(150)))
		{
			...
			for(zz = 0; zz < bp.btFoldouts.Count;zz++)
	...
	function BulletTagActionsGUI(i)
	{
		...
		if (actions.Count != bp.btaFoldouts[i].sub.Count)
			bp.btaFoldouts[i].sub = new List.<boolean>(new boolean[actions.Count]);
				...
				if (bp.btaFoldouts[i].sub[l]) 
				{
					...
					switch(ac.type)
					{
						...
						case(BulletActionType.StartRepeat):
							ac.repeatCount.x = EditorGUILayout.IntField("Repeat Count", ac.repeatCount.x);
							GUILayout.BeginHorizontal();
							ac.rankRepeat = EditorGUILayout.Toggle("AddRank", ac.rankRepeat);
							if(ac.rankRepeat)
